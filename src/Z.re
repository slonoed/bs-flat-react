...
module React = {
    let createElement = (cmp, props, children): reactElement => {
        [%raw "require('react').createElement(cmp, props, ...children)"]
    };
};
module ReactDOM = {
    [@bs.module "react-dom"] external render : (reactElement, 'a) => unit = "render";
};

let render = (el: reactElement, domNode) => {
    ReactDOM.render(el, domNode);
};

type component('a) = ('a) => reactElement;


/* create element from tag */
let createElement: (string, props, list(reactElement)) => reactElement = (tag, props, c) => {
    let children = Array.of_list(c);
    let result = React.createElement(tag, props, children);
...
let r: ('p => reactElement, 'p, list(reactElement)) => reactElement = (foo, p, children) => {
    React.createElement(foo, p, Array.of_list(children));
};

let t = createElement;

let text: (string) => reactElement = [%raw {|
    (a => a)
|}];

let extractChildren = (a): reactElement => {
